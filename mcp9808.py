...
class MCP9808(object):
    ...
    def set_alert_mode(
        ...
        alert_bits = (
            output_mode | (polarity << 1) | (selector << 2) | (enable_alert << 3)
        ) & 0xF
    ...
    def set_alert_boundary_temp(self, boundary_register, value):
        ...
        """
        if boundary_register not in [
            REG_TEMP_BOUNDARY_LOWER,
        ...
        frac = (
            ((1 if frac * 2 >= 1 else 0) << 1)
            + (1 if (frac * 2 - int(frac * 2)) * 2 >= 1 else 0)
        ) << 2
